...
const StatusBadge: FC<StatusBadgeProps> = ({ currentStep }) => {
    ...
    const currentStepFormated = useMemo(() => {
        return translatedText['current_step_' + currentStep]?.message || currentStep;
    }, [currentStep, translatedText]);

    return (
        <span className="inline-flex items-center py-1 px-2 bg-gray-800 border border-gray-600 text-heading text-xs font-medium rounded-lg gap-1 w-fit">
            ...
            <span className="text-white capitalize -translate-y-px">{ currentStepFormated }</span>
        </span>
    );
};
